...
import type { MeterTaskParam } from '../../../types/csgframe';

interface MeterTaskFrameProps {
...
const MeterTaskFrame: React.FC<MeterTaskFrameProps> = ({ onFrameGenerator, initialParams }) => {
    const { generatorMeterTaskFrame } = useWasm();
    ...
    const [taskParam, setTaskParam] = useState<MeterTaskParam>(() => {
        if (initialParams) {
            return {
                ...initialParams,
                // 确保时间戳是数字类型
                report_base_time: initialParams.report_base_time || getCurrentTime(),
                meter_read_base_time: initialParams.meter_read_base_time || getCurrentTime(),
        ...
        return {
            ...
            report_cycle: 1,
            data_struct: 0,
            meter_read_base_time: getCurrentTime(),
            meter_read_unit: 0,
            meter_read_cycle: 1,
            data_rate: 1,
    ...
    const handleInputChange = (field: keyof MeterTaskParam, value: string | number | boolean) => {
        setTaskParam(prev => ({
            ...prev,
            [field]: value
        }));
    ...
    const handleTimeChange = (field: 'report_base_time' | 'meter_read_base_time' | 'read_base_time', value: string) => {
    ...
    const generateFrame = useCallback(async () => {
        try {
            ...
            if (taskParam.meter_read_base_time === 0) {
                toast.error('请配置表端采样基准时间');
                return;
            }
            if (taskParam.read_base_time === 0) {
                toast.error('请配置终端采样基准时间');
                return;
            }

            const hexString = await generatorMeterTaskFrame(taskParam);
            ...
            toast.success('电表任务报文生成成功');

            console.log('电表任务参数:', taskParam);
        } catch (error) {
            console.error('生成电表任务报文失败:', error);
            toast.error(`生成报文失败: ${error instanceof Error ? error.message : '未知错误'}`);
        }
    }, [taskParam, generatorMeterTaskFrame, onFrameGenerator]);

    return (
        <div className="h-full flex flex-col">
            <div className="card bg-base-100 shadow-xl h-full flex flex-col">
                <div className="card-body flex flex-col h-full">
                    <h2 className="card-title text-primary mb-4">表端任务参数配置</h2>
                    ...
                    <div className="flex-1 overflow-auto mb-6">
                        <div className="space-y-4 px-2">
                            ...
                            {/* 表端采样配置 */}
                            <div className="divider">表端采样配置</div>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className="form-control flex flex-col w-full">
                                    <label className="label label-text text-xs">
                                        <span className="label-text font-semibold">表端采样基准时间</span>
                                    </label>
                                    <input
                                        type="datetime-local"
                                        className="input input-bordered input-primary w-full"
                                        value={formatDateTimeLocal(taskParam.meter_read_base_time)}
                                        onChange={(e) => handleTimeChange('meter_read_base_time', e.target.value)}
                                    />
                                </div>

                                <div className="form-control flex flex-col w-full">
                                    <label className="label label-text text-xs">
                                        <span className="label-text font-semibold">表端采样周期单位</span>
                                    </label>
                                    <select
                                        className="select select-bordered select-primary w-full"
                                        value={taskParam.meter_read_unit}
                                        onChange={(e) => handleInputChange('meter_read_unit', parseInt(e.target.value))}
                                    >
                                        {Object.entries(ReportUnit).map(([key, value]) => (
                                            <option key={key} value={key}>
                                                {value}
                                ...
                                <div className="form-control flex flex-col w-full">
                                    <label className="label label-text text-xs">
                                        <span className="label-text font-semibold">表端采样周期</span>
                                    </label>
                                    <input
                                        type="number"
                                        className="input input-bordered input-primary w-full"
                                        value={taskParam.meter_read_cycle}
                                        onChange={(e) => {
                                            const value = parseInt(e.target.value);
                                            handleInputChange('meter_read_cycle', isNaN(value) ? 0 : value);
                                        }}
                                        min="0"
                                        max="255"
                                    />
                            ...
                            {/* 终端采样配置 */}
                            <div className="divider">终端采样配置</div>
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div className="form-control flex flex-col w-full">
                                    <label className="label label-text text-xs">
                                        <span className="label-text font-semibold">终端采样基准时间</span>
                                    </label>
                                    <input
                                        type="datetime-local"
                                        className="input input-bordered input-primary w-full"
                                ...
                                <div className="form-control flex flex-col w-full">
                                    <label className="label label-text text-xs">
                                        <span className="label-text font-semibold">终端查询周期单位</span>
                                    </label>
                                    <select
                                        className="select select-bordered select-primary w-full"
                                ...
                                <div className="form-control flex flex-col w-full">
                                    <label className="label label-text text-xs">
                                        <span className="label-text font-semibold">终端查询周期</span>
                                    </label>
                                    <input
                                        type="number"
                                        className="input input-bordered input-primary w-full"
