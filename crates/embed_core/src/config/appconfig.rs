...
// 如果构建既不是 desktop 也不是 wasm，提供后备的 stub 实现，避免上层模块在无特性构建时报错
#[cfg(all(not(feature = "desktop"), not(feature = "wasm")))]
pub fn load_config_value(_section: &str, _key: &str) -> Option<Value> {
    None
}

#[cfg(all(not(feature = "desktop"), not(feature = "wasm")))]
pub fn set_config_value(_section: &str, _key: &str, _value: &str) -> Result<(), String> {
    Ok(())
}

// wasm 平台下使用 localStorage 实现配置读写，并导出给 JS 调用
#[cfg(feature = "wasm")]
pub fn load_config_value(section: &str, key: &str) -> Option<Value> {
    let window = web_sys::window()?;
    let storage = window.local_storage().ok().flatten()?;
    let contents = storage.get_item(constants::APP_CONFIG).ok().flatten()?;
    let root: Value = serde_json::from_str(&contents).ok()?;
    if key.is_empty() {
        root.get(section).cloned()
    } else {
        root.get(section).and_then(|s| s.get(key)).cloned()
    }
}
...
pub fn set_config_value(section: &str, key: &str, value: &str) -> Result<(), String> {
    ...
    let storage = window
        .local_storage()
        .map_err(|e| format!("local_storage error: {:?}", e))?
        .ok_or("no localStorage")?;

    let mut root = match storage.get_item(constants::APP_CONFIG).map_err(|e| format!("get_item error: {:?}", e))? {
        Some(s) => serde_json::from_str(&s).map_err(|e| format!("parse error: {}", e))?,
        None => Value::Object(serde_json::Map::new()),
    };

    let value_json: Value = serde_json::from_str(value).map_err(|e| format!("Failed to parse JSON: {}", e))?;

    if key.is_empty() {
        if let Some(map) = root.as_object_mut() {
            map.insert(section.to_string(), value_json);
        } else {
            return Err("root not object".to_string());
        }
    } else {
        if let Some(map) = root.as_object_mut() {
            let section_val = map
                ...
                .or_insert_with(|| Value::Object(serde_json::Map::new()));
            if let Value::Object(sec_map) = section_val {
                sec_map.insert(key.to_string(), value_json);
            } else {
                return Err("section not object".to_string());
            }
        } else {
            return Err("root not object".to_string());
        }
    ...
    let payload = serde_json::to_string(&root).map_err(|e| format!("serialize error: {}", e))?;
    storage
        .set_item(constants::APP_CONFIG, &payload)
        .map_err(|e| format!("set_item error: {:?}", e))?;
    Ok(())
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn wasm_set_config_value(section: &str, key: &str, value: &str) -> Result<(), JsValue> {
    set_config_value(section, key, value).map_err(|e| JsValue::from_str(&e))
}

#[cfg(feature = "wasm")]
#[wasm_bindgen]
pub fn wasm_get_config_value(section: &str, key: &str) -> JsValue {
    match load_config_value(section, key) {
        Some(v) => JsValue::from_str(&serde_json::to_string(&v).unwrap_or_default()),
        None => JsValue::NULL,
    }
}
